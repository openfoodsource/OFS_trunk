...
CREATE TABLE IF NOT EXISTS ofs_membership_types (
  ...
  initial_cost float NOT NULL COMMENT 'initial membership cost for this membership level',
  order_cost float NOT NULL COMMENT 'additionally charge this amount on each order (modified by order_cost_type)',
  order_cost_type enum('fixed','percent') NOT NULL DEFAULT 'fixed',
  customer_fee_percent decimal(6,3) NOT NULL DEFAULT '0.000' COMMENT 'set this customer fee percent',
  producer_fee_percent decimal(6,3) NOT NULL DEFAULT '0.000' COMMENT 'set this producer fee percent if creating a producer account',
  membership_class varchar(30) NOT NULL,
  membership_description varchar(255) NOT NULL,
  pending tinyint(4) NOT NULL DEFAULT '1' COMMENT '0: start this membership as non-pending; 1: start as pending',
  enabled_type tinyint(4) NOT NULL COMMENT 'Binary allow new/existing members at this level 0=none [00]; 1=new only [01]; 2=existing only [10]; 3=anyone [11]',
  may_convert_to varchar(255) NOT NULL COMMENT 'membership_type_id that this membership_type may convert into',
  renew_cost float NOT NULL COMMENT 'cost to renew a this membership level',
  expire_after int(11) NOT NULL COMMENT 'force membership renewal after this period (modified by expire_type)',
