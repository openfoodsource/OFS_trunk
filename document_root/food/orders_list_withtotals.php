...
$result = @mysqli_query ($connection, $query) or die (debug_print ("ERROR: 735933 ", array ($query, mysqli_error ($connection)), basename(__FILE__).' LINE '.__LINE__));
...
while ( $row = mysqli_fetch_array ($result, MYSQLI_ASSOC) )
  {
    ...
    $query_total = '
      ...
      WHERE
        ...
        AND (
            (source_key = "'.mysqli_real_escape_string ($connection, $member_id).'"
    ...
    $result_total = @mysqli_query ($connection, $query_total) or die (debug_print ("ERROR: 281033 ", array ($query_total, mysqli_error ($connection)), basename(__FILE__).' LINE '.__LINE__));
    ...
    $query_estimate = '
      ...
      WHERE
        ...
        AND member_id="'.mysqli_real_escape_string ($connection, $member_id).'"';
    $result_estimate = @mysqli_query ($connection, $query_estimate) or die (debug_print ("ERROR: 785103 ", array ($query_estimate, mysqli_error ($connection)), basename(__FILE__).' LINE '.__LINE__));
    ...
    $resultprp = @mysqli_query ($connection, $sqlp) or die (debug_print ("ERROR: 782933 ", array ($sqlp, mysqli_error ($connection)), basename(__FILE__).' LINE '.__LINE__));
    ...
    $display .= '</td>
      <td class="member_links">'.
        (CurrentMember::auth_type('member_admin') == true ? '<a class="popup" onclick="popup_src(\'edit_member.php?action=edit&member_id='.$member_id.'&display_as=popup\', \'edit_producer\', \'\', false)">Edit</a>' : '').
        (CurrentMember::auth_type('member_admin') == true && CurrentMember::auth_type('cashier') == true ? ' | ' : '').
        (CurrentMember::auth_type('cashier') == true ? '<a class="popup" onclick="popup_src(\'member_information.php?member_id='.$member_id.'&display_as=popup\', \'edit_member\', \'\', false)">View</a>' : '').'
      </td>
      <td class="order_links" valign="top"><a href="product_list.php?type=customer_basket&amp;delivery_id='.$delivery_id.'&amp;member_id='.$member_id.'&amp;basket_id='.$basket_id.'">Basket</a>&nbsp;|&nbsp;<a href="show_report.php?type=customer_invoice&amp;delivery_id='.$delivery_id.'&amp;member_id='.$member_id.'">Invoice</a></td>
    </tr>';
